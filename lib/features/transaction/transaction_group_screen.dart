...
class TransactionGroupScreen extends StatefulWidget {
  final Group group;
  const TransactionGroupScreen({super.key, required this.group});
...
class _TransactionGroupScreenState extends State<TransactionGroupScreen> {
  ...
  Future<void> fetchTransactions() async {
    ...
    try {
      final response = await _groupService.getAllTransactionsByGroup(widget.group.id!, token, context);
      ...
      if (query == null || query.isEmpty) {
        ...
        transactions = transactions.where((transaction) {
          return transaction.amount.toString().contains(query) ||
              ...
              transaction.member.user!.username.toString().contains(query) ||
